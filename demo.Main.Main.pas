...
begin
  if (DemoMain <> nil) and (FormMain<>nil) and DemoMain.AppCloseTerminated then
...
begin
  ...
  FAppCloseLockCount:=0;
   ...
   LogMain:= TAmLogString.CreateLogCustom(nil,
                                         AmFile.LocalPath+MainPathLog+'log.log',
                                         AmFile.LocalPath+MainPathLog+'\log.log',
                                         AmFile.LocalPath+MainPathLog+'logErrorApp.log',
                                         AmFile.LocalPath+MainPathLog+'logErrorCollector.log',
                                         nil);
  LogMain.Log('================================================================================');
...
procedure TdMain.Notification(Source: TAmObjectNotify; Msg: TAmOperation; W,
  L: Cardinal);
begin
  inherited;
...
begin
   Result:=
   AmFile.IsFile(AmFile.LocalPath + ws_ssl_server_key_fn)
...
begin
    Result:= FFormCodeEditor; //Form:= AmPaxFormCodeEditor.New
end;
...
begin
    inherited;
    FModuleApiVk:= TdModuleApiVkBazaSetting.Create(AObj['ModuleApiVk'].ObjectValue);
    FModuleWebSocketClient:=TdModuleWebSocketClientSetting.Create(AObj['ModuleWebSocketClient'].ObjectValue);
    FModuleWebSocketServer:=TdModuleWebSocketServerSetting.Create(AObj['ModuleWebSocketServer'].ObjectValue);
...
begin
  Result:= AmAtomic.Getter<TdModuleWebSocketClientSetting>(FModuleWebSocketClient);
end;

function TdBazaSetting.ModuleWebSocketServerGet: TdModuleWebSocketServerSetting;
begin
   Result:= AmAtomic.Getter<TdModuleWebSocketServerSetting>(FModuleWebSocketServer);
end;
